...
export default function ResearchPage() {
  ...
  const [activeTab, setActiveTab] = useState<"overview" | "financials" | "news" | "deep-research">("overview")
  ...
  const saveDeepResearch = async () => {
    if (!companyName || !deepResearchData) return
      ...
      case "overview":
        return (
          <div className="space-y-8 mt-6">
            {/* Add the comparison section at the top */}
            {saved && (
              <motion.div
                initial={{ opacity: 0, y: 20 }}
                animate={{ opacity: 1, y: 0 }}

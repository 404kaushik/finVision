...
const sampleStocks: Stock[] = [
  ...
  { symbol: "AMZN", companyName: "Amazon.com Inc.", price: 127.74, changePercent: 0.95, change: 1.20 },
  { symbol: "NVDA", companyName: "NVIDIA Corp.", price: 416.10, changePercent: 2.35, change: 9.56 },
  { symbol: "META", companyName: "Meta Platforms Inc.", price: 297.74, changePercent: 1.15, change: 3.38 },
  { symbol: "TSLA", companyName: "Tesla Inc.", price: 237.49, changePercent: -0.75, change: -1.80 },
...
export function StockCarousel({ title, description, filter }: StockCarouselProps) {
  ...
  const nextSlide = () => {
    ...
    setCurrentIndex((prevIndex) => 
      prevIndex === filteredStocks.length - 3 ? 0 : prevIndex + 1
    )
  }
  ...
  const prevSlide = () => {
    if (filteredStocks.length <= 3) return
    setCurrentIndex((prevIndex) => 
      prevIndex === 0 ? filteredStocks.length - 3 : prevIndex - 1
    )
  }
  ...
  return (
    <div className="w-full bg-card-bg/30 backdrop-blur-sm rounded-xl p-6 shadow-lg border border-white/10">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          {visibleStocks.map((stock) => (
            <motion.div
              ...
              className="bg-card-hover/30 backdrop-blur-sm p-4 rounded-lg border border-white/5 hover:border-primary/20 transition-all hover:shadow-lg group"
            >
              <div className="flex justify-between items-start">
                <div>
                  <h3 className="font-bold text-lg group-hover:text-primary transition-colors">{stock.symbol}</h3>
                  <p className="text-sm text-muted-foreground truncate max-w-[180px]">{stock.companyName}</p>
                </div>
